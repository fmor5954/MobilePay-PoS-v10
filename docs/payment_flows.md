...
cancelled by MobilePay the state transitions to *CancelledByMobilePay*. If a payment includes age restrictions that prevents the customer from completing the payment, the payment is rejected by MobilePay and the state transistions to *RejectedByMobilePayDueToAgeRestrictions*. Payments without age restrictions will never transition to the *RejectedByMobilePayDueToAgeRestrictions* state.
